...
class Datasets(Dataset):
    ...
    def __init__(self, opt, actions=None, split=0):
        ...
        :param sample_rate:
        """
        self.path_to_data = "../data/h3.6m/"
        self.split = split  # 划分方式，决定本次数据集是训练集、测试集还是验证集
        self.in_n = opt.input_n  # 输入的帧数
        self.out_n = opt.output_n  # 预测的帧数
        self.sample_rate = 2  # 每间隔多少帧取一帧
        self.p3d = {}
        self.data_idx = []
        device = get_device()
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 2:
                    for subact in [1, 2]:  # subactions
                        ...
                        the_sequence = data_utils.readCSVasFloat(
                            filename)  # 读取文件
                        n, d = the_sequence.shape  # 序列的长度、维度
                        even_list = range(0, n, self.sample_rate)  # 采样的下标
                        num_frames = len(even_list)  # 采样总帧数
                        the_sequence = np.array(the_sequence[even_list, :])
                        the_sequence = torch.from_numpy(
                            the_sequence).float().to(device)
                        # remove global rotation and translation
                        the_sequence[:, 0:6] = 0
                        p3d = data_utils.expmap2xyz_torch(
                            the_sequence)  # 四元数的映射转换到xyz坐标表示
                        # self.p3d[(subj, action, subact)] = p3d.view(num_frames, -1).cpu().data.numpy()
                        self.p3d[key] = p3d.view(
        ...
        self.dimensions_to_use = np.setdiff1d(
            np.arange(96), dimensions_to_ignore)
