...
def main(opt):
    ...
    net_pred.to(device=get_device())
    model_path_len = '{}/ckpt_best.pth.tar'.format(opt.ckpt)
    print(">>> loading ckpt len from '{}'".format(model_path_len))
    ckpt = torch.load(model_path_len, map_location=get_device())
    ...
    log.save_csv_log(opt, head, value, is_create=True,
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None, dim_used=None):
    ...
    joint_used = np.array([1,2,3,4,6,7,8,9,11,12,13,14,15,17,18,20,21,24,25,26,28,29])
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all = net_pred(p3d_src, input_n=in_n,
                                output_n=10, itera=itera)

        p3d_out_all = p3d_out_all[-1][:, seq_in:].transpose(
            1, 2).reshape([batch_size, 10 * itera, -1])[:, :out_n]
