...
def main(opt):
    assert opt.output_n % opt.sub_output_n == 0
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        for j in range(opt.num_stage):
            if j == 0:
                ...
                smooths.append(smooth(smooths[-1], sample_len=opt.kernel_size + opt.output_n,
                                      kernel_size=opt.kernel_size).clone())
        p3d_sups = []
        # [[smooth_n], [smooth_n-1],..., [raw]]
        for j in range(opt.num_stage):
            p3d_sups.append(smooths[-(j+1)].clone()[:, -out_n - seq_in:].reshape(
        ...
        for j in range(opt.num_stage):
            p3d_out_alls[j] = p3d_out_alls[j].reshape(
                [batch_size, seq_in + out_n, 1, len(dim_used) // 3, 3])
